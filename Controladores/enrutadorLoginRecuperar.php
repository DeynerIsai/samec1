...
/*namespace SAMEC\controladorLoginRecuperar;*/

include_once '../Plugins/Libs/helper.php';
include_once '../Plugins/Model/User.php';

/*include_once '../PHPMailer/PHPMailer/Exception';
include_once '../PHPMailer/PHPMailer/PHPMailer';*/

class controladorLoginRecuperar
{
    public function index()
    {
        $this->render('../Vistas/AdministrativaLogin.php', 'Iniciar Sesión', null, false);
    }
    ...
    {
        $this->render('../Vistas/template.php', 'Registrar Usuario', null, false);
    }

    public function recover()
    {
        $this->render('../Vistas/AdministrativaRecuperarCuenta.php', 'Recuperar Contraseña', null, false);
    }
    ...
    public function sendRecoveryCode()
    {
        if (isset($_POST["txtCorreoElectronico"]) && trim($_POST["txtCorreoElectronico"] != '')) {
            ...
            if ($user === false) {
                ...
                $this->render('../Vistas/AdministrativaRecuperarCuenta.php', 'Recuperar Contraseña', array('mensaje' => $mensaje), false);
            } else {
        ...
        if (isset($code)) {
            ...
            if ($user === false) {
                ...
                if (strtotime($current) > strtotime($user->fechaRecuperacion)) {
                    ...
                    $this->render('../Vistas/AdministrativaNuevaContraseña.php', 'Nueva Contraseña', array('user' => $user), false);
                }
            }
        } else {
            header('location: ' . URL);
    ...
    public function updatePasswordWithCode()
    {
        if (isset($_POST['btnGuardar'])) {
            ...
            if ($contrasena != $repetirContrasena) {
                ...
                $this->render('../Vistas/AdministrativaNuevaContraseña.php', 'Registrar Usuario', array('user' => $user, 'mensaje' => $mensaje), false);
                return;
                ...
                if ($resultado != false) {
                    ...
                    $this->render('../Vistas/AdministrativaLogin.php', 'Iniciar Sesion', array('mensaje' => $mensaje), false);
                    return;
    ...
    {
        $template = file_get_contents(APP . '../Vistas/template.php');
        $template = str_replace("{{name}}", $nombre, $template);
        $template = str_replace("{{action_url_2}}", '<b>http:' . URL . '../Vistas/AdministrativaNuevaContraseña.php' . $codigo . '</b>', $template);
        $template = str_replace("{{action_url_1}}", 'http:' . URL . '../Vistas/AdministrativaNuevaContraseña.php' . $codigo, $template);
        ...
        try {
            ...
            $mail->Username   = 'isc.deynerisai@gmail.com'; //username
            $mail->Password   = 'deynerisaitec'; //password
            $mail->SMTPSecure = 'ssl';
            $mail->Port       = 465; //smtp port

            $mail->setFrom('isc.deynerisai@gmail.com', 'Samec');
            $mail->addAddress($correoElectronico, $nombre);

            $mail->isHTML(true);

            $mail->Subject = 'Recuperación de contraseña - Samec';
            $mail->Body    = $template;
